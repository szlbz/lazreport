...
type
  ...
  public
    ...
    Selected: Boolean;
    OriginalRect: TRect;
    ScaleX, ScaleY: Extended;   // used for scaling objects in preview
    OffsX, OffsY: Integer;    //
    IsPrinting: Boolean;
    Flags: Word;
    DRect: TRect;
  ...
  private
    ...
    FBlankDispText:String; //2022.03.15 LBZ
  ...
  public
    ...
    property BlankDispText :String read FBlankDispText write FBlankDispText;//2022.03.15 LBZ
  end;
  ...
  published
    ...
    property GapX;
    property GapY;
    property BlankDispText;  //2022.03.15 LBZ
  end;
  ...
  private
    fLinesPerPage: Word; //2022.03.27 LBZ
    fDataSetStr : String;
  ...
  published
    ...
    property LinesPerPage: Word read  fLinesPerPage write fLinesPerPage; //2022.03.27 LBZ 增加每页行数
  ...
  private
    ...
    FNowLine: LongInt;//2022.03.15 LBZ
  ...
  public
    LinesPerPages: Word;  //2022.03.27 LBZ
    EOFReached: Boolean;
    MaxDY: Integer;
  ...
  private
    ...
    FWaterMarkText: String;//2022.06.14 LBZ
  ...
  published
    ...
    property WaterMarkText: String read fWaterMarkText write fWaterMarkText;//2022.06.14 LBZ
  end;
...
//    FDefaultTitle : String;
  ...
  public
    ...
    procedure PrintReport;//LBZ 2022.06.12
...
begin
  ...
  InternalGapX := wx2 + 1 + round(FGapX*ScaleX);  //2024.1.12 LBZ
  InternalGapY := wy2 + 1 + Round(FGapY*ScaleY);
...
begin
  ...
  begin
    ...
    if CurReport.WaterMark.Text<>'' then Exit;//2021.06.14 LBZ 当打印水印时，取消背景颜色，防止遮挡水印。
    if CurPage.WaterMarkText<>'' then Exit;//2021.06.14 LBZ 当打印水印时，取消背景颜色，防止遮挡水印。
  end;
  ...
  StreamMode := TfrStreamMode(XML.GetValue(Path+'StreamMode/Value'{%H-}, 0)); // TODO Check default
{
  ...
  FFillColor := StringToColor(XML.GetValue(Path+'FillColor/Value','clNone'));//2022.06.14 LBZ將默认的'clWindow'改为'clNone')); // TODO Check default
  if StreamMode = smDesigning then
  begin
...
begin
  ...
  Flags := flStretched + flWordWrap+flWordBreak;//2022.03.15 LBZ
  BlankDispText:=''; //2024.06.27 LBZ
  LineSpacing := 2;
  CharacterSpacing := 0;
  Adjust := 0;
end;
...
begin
  ...
  if HideZeroValues<>AValue then  //2021.12.19 LBZ
...
begin
  ...
  begin
    ...
    FBlankDispText := TfrCustomMemoView(Source).FBlankDispText;//2022.03.15 LBZ
...
function textstr(s:string):string; //清除字符串的上下标标志
begin
  //#02<sup></sup>  #03<sub></sub> #04取消上下标粗体斜体 #05粗体<b></b>  #06下划线<u></u>  #07斜体<i></i>
  s:=s.Replace(#02,'',[rfReplaceAll]); //#02 上标
  s:=s.Replace(#03,'',[rfReplaceAll]);//#03 下标
  s:=s.Replace(#04,'',[rfReplaceAll]); //#04 取消上下标斜体
  s:=s.Replace(#05,'',[rfReplaceAll]); //#05 粗体字
  s:=s.Replace(#06,'',[rfReplaceAll]); //#06 下划线
  s:=s.Replace(#07,'',[rfReplaceAll]); //#06 斜体
  Result:=s;
end;
...
var
  ...
  procedure WrapLine(s: String);//LBZ
  var
    i, cur, beg, last, len: Integer;
    WasBreak, CRLF, IsCR: Boolean;
    ch: char;
    FL:Boolean;//2024.01.04 LBZ
    pgap:Integer;//2024.01.04 LBZ
  begin
    //换行<br> #02上标<sup></sup>  #03下标<sub></sub> #04取消上下标粗体斜体 #05粗体<b></b>  #06下划线<u></u>  #07斜体<i></i>
    s:=s.Replace('<sup>',#02,[rfReplaceAll]); //#02 上标  //#03 下标 //#04 取消上下标粗体斜体 //#05 粗体  #06 下划线 #07 斜体
    s:=s.Replace('</sup>',#04,[rfReplaceAll]);//#03 下标
    s:=s.Replace('<sub>',#03,[rfReplaceAll]); //#04 取消上下标粗体斜体
    s:=s.Replace('</sub>',#04,[rfReplaceAll]);//#05 粗体
    s:=s.Replace('<b>',#05,[rfReplaceAll]);   //#06 下划线
    s:=s.Replace('</b>',#04,[rfReplaceAll]);  //#07 斜体
    s:=s.Replace('<u>',#06,[rfReplaceAll]);
    s:=s.Replace('</u>',#04,[rfReplaceAll]);
    s:=s.Replace('<i>',#07,[rfReplaceAll]);
    s:=s.Replace('</i>',#04,[rfReplaceAll]);
    CRLF := False;
    for i := 1 to UTF8Length(s) do//LBZ
    begin
    ...
    last := 1; beg := 1;
    //LBZ
    if not CRLF and ((UTF8Length(textstr(s)) <= 1) or
      ((WCanvas.TextWidth(textstr(s)) + round(WCanvas.TextWidth('国')*FParagraphGap)) <= maxwidth)) then
    ...
    begin
      ...
      cur := 1;
      Len := UTF8Length(s); //LBZ
      FL:=true; //2024.01.04 LBZ   OutLine
      ...
      begin
        ...
        if Ch=#10 then
        begin
          OutLine(UTF8copy(s, beg, cur - beg) + #1);  //LBZ
        ...
        if fl then
          pgap:=round(WCanvas.TextWidth('国')*FParagraphGap)//*ScaleX)//2024.01.04 LBZ
        else
          pgap:=0;
        if (ch<>' ') and ((WCanvas.TextWidth(textstr(UTF8copy(s, beg, cur - beg + 1)))+pgap) > maxwidth) then//2024.01.04 LBZ
        ...
        begin
          ...
          begin
            ...
            if UTF8Length(b) > 0 then
            begin
              i := 1;
              while (i <= UTF8Length(b)) and
  ...
  procedure outmemo1(s: string);  // 2021.12.21 LBZ <br>--强制换行
  var
    i, j: Integer;
  ...
  begin
    ...
    begin
      if (s2[i] = '<') and (s2[i+1]='b') and (s2[i+2]='r') and (s2[i+3]='>') then
      begin
        Inc(j);
        a[j] := i;
      end;
    ...
    begin
      ...
      begin
        ...
        begin
          if i = j + 1 then
            s1 := (Copy(s, a[i - 1] + 4, Length(s) - a[i]))
          else
            s1 := (Copy(s, a[i - 1] + 4, a[i] - a[i - 1] - 4));
        end;
        WrapLine(s1);
      end;
    end;
...
//    maxWidth := dx - gapx - gapx;
    ...
    begin
      ...
      //else
        //OutLine(Memo1[i] + #1);
      outmemo1(Memo1[i]); // 2021-12-21 LBZ 增加强制换行功能
    end;
...
var
  ...
  var
    c1, fs, fs1, i, j, oldY, oldFont: Integer;
    str,ss: string;
    fonts: Integer;
    fontsizes:integer;
    AStyle: TTextStyle;
  begin
    ...
    begin
      ...
      begin
        ...
        if c <> 0 then
          c1 := fs - fs1; // 两端对齐时
        y := y + Round(((fs / 2) + 2) * ScaleY);
        UTF8Delete(st, i, 1);
      end;
      if ss = #04 then // 取消上下标/粗体字/下划线
      begin
        c1 := 0;
        Canvas.Font.Size := fs;
        Canvas.Font.Style:=Canvas.Font.Style- [fsBold]-[fsUnderline]-[fsItalic];//
        y := oldY;
        UTF8Delete(st, i, 1);
      end;
      if ss = #05 then // 粗体字
  ...
  var
    ...
    begin
      ...
      //#06下划线<u></u>
      //#07斜体<i></i>
      st:=st.Replace('<sup>',#02,[rfReplaceAll]);
      st:=st.Replace('</sup>',#04,[rfReplaceAll]);
      st:=st.Replace('<sub>',#03,[rfReplaceAll]);
      st:=st.Replace('</sub>',#04,[rfReplaceAll]);
      st:=st.Replace('<b>',#05,[rfReplaceAll]);
      st:=st.Replace('</b>',#04,[rfReplaceAll]);
      st:=st.Replace('<u>',#06,[rfReplaceAll]);
      st:=st.Replace('</u>',#04,[rfReplaceAll]);
      st:=st.Replace('<i>',#07,[rfReplaceAll]);
      st:=st.Replace('</i>',#04,[rfReplaceAll]);
      ...
      begin
        ...
        *)
        ...
        begin
          if FirstLine then
            showchar(round(CurX), round(CurYf),0,round(Canvas.TextWidth('国')*FParagraphGap), st) // 2002.2.8 LBZ
          else
            showchar(round(CurX), round(CurYf),0, 0, st); // 2002.2.8 LBZ
        end
        else
        begin
          if FirstLine then
        ...
        end;
{
      ...
      //if Length(St)>0 then
      curyf := curyf + thf+LineSpc; //2024-07-02 LBZ
      FirstLine:=FTmpFL;
    end;
  ...
  begin {OutMemo}
    ...
    begin
      if Layout=tlCenter then  //垂直居中
      //{$ifdef linux}
      //  if Printer.Printing then //2022.12.14 LBZ修正打印和预览垂直居中不正确的Bug
      //    y := y + (dy-VHeight) div 2 -abs(Self.Font.Height div 2)
      //  else
      //    y := y + (dy-VHeight) div 2 -round(InternalGapY*2*ScaleY)//2022.06.14 LBZ修正垂直居中的Bug
      //{$else}
        y := y + (dy-VHeight) div 2
      //{$endif}
      else
      if Layout=tlBottom then
        y:=y+dy-VHeight-round(FGapY*ScaleY);//2024.01.12 LBZ
    end;
    curyf := y + FGapY*ScaleY;// InternalGapY; //2024.01.12 LBZ
    ...
    if Self.Font.Size = 0 then
      h := Round((-GetFontData(Self.Font.Handle).Height * 72 / Self.Font.PixelsPerInch))
    else
      h := Self.Font.Size;
  ...
  function wides(s:string):integer;
  var
  CalcRect: TRect;
  n: Integer;
  DTFlags: Cardinal;
  ...
  begin
    j := 0;
    setlength(wd,Memo1.Count);
    wds:=wides(Memo1[0]);
    for i := 0 to Memo1.Count - 1 do
    begin
      wd[i]:=wides(Memo1[i]);
      if wds<wd[i] then wds:=wd[i];
    end;
    wd:=nil;
    Result :=wds ;

  end;
// 2021.12.21计算memo最长的行宽。LBZ

//Begin行高度计算
  function hid: Integer;
  var h,hs,hds:integer;
  begin
     hs:=Memo1.Count;
     h:= Round((FFont.Size*96 / 72));
    hds :=round((h*hs+LineSpacing*hs)* ScaleY)+GapY*2;
    Result:=hds;
  end;
//END行高度计算
...
begin
  ...
  SavY:=Y;
  Try
    //SetTextCharacterExtra(Canvas.Handle, Round(CharacterSpacing * ScaleX));//2024-07-02 LBZ 删除
    DR:=Rect(DRect.Left + 1, DRect.Top, DRect.Right - 2, DRect.Bottom - 1);
    VHeight:=Round(VHeight*ScaleY);
    // 2021.12.21 当字体缩放=true AND 自动换行=false时,自动单行字体缩放 LBZ
    if  ((Flags and flWordWrap) = 0) then
      if Memo1.Count > 0 then
      begin
        while ((wid > abs(DRect.Left - DRect.Right)+ Round(2 * FrameWidth)) or (hid>DRect.Height)) and (Canvas.Font.Size > 5) do
        begin
          Canvas.Font.Size := Canvas.Font.Size - 1;
          FFont.Size := Canvas.Font.Size;
        end;
      end;
    // 2021.12.21 当字体缩放=true AND 自动换行=false时,自动单行字体缩放 LBZ
...
begin
  ...
  s := aMemo.Text;
  //#02<sup></sup>  #03<sub></sub> #04取消上下标粗体斜体 #05粗体<b></b>  #06下划线<u></u>  #07斜体<i></i>
  s:=s.Replace('<sup>',#02,[rfReplaceAll]);
  s:=s.Replace('</sup>',#04,[rfReplaceAll]);
  s:=s.Replace('<sub>',#03,[rfReplaceAll]);
  s:=s.Replace('</sub>',#04,[rfReplaceAll]);
  s:=s.Replace('<b>',#05,[rfReplaceAll]);
  s:=s.Replace('</b>',#04,[rfReplaceAll]);
  s:=s.Replace('<u>',#06,[rfReplaceAll]);
  s:=s.Replace('</u>',#04,[rfReplaceAll]);
  s:=s.Replace('<i>',#07,[rfReplaceAll]);
  s:=s.Replace('</i>',#04,[rfReplaceAll]);
...
begin
  ...
  begin
    ...
    begin
      ...
      FBlankDispText:=frReadString(Stream);//2022.06.29 LBZ
    end;
  end;
...
begin
  ...
  FBlankDispText:=XML.GetValue(Path+'Data/BlankDispText/Value', '');//2022.03.15 LBZ
end;
...
begin
  ...
  begin
    ...
    frWriteString(Stream,FBlankDispText);//2022.06.29 LBZ
  end;
end;
...
begin
  ...
  XML.SetValue(Path+'Data/BlankDispText/Value', FBlankDispText);//2022.03.15 LBZ
end;
...
begin
  ...
  fFormat := 0;
  BaseName := 'Band';
  fLinesPerPage := 0; //2022.03.27 LBZ
...
begin
  ...
  begin
    ...
    FLinesPerPage := TFrBandView(Source).fLinesPerPage; //2022.03.27 LBZ
...
begin
  ...
  if frVersion>23 then begin
    ...
    begin
      ...
      fLinesPerPage :=strtoint(ReadString(Stream));//2022.03.27 LBZ
    end;
...
begin
  ...
  fLinesPerPage := XML.GetValue(Path+'LinesPerPage/Value', 0); //2022.03.27 LBZ
  FixPrintChildIfNotVisible;
end;
...
begin
  ...
  begin
    ...
    frWriteString(Stream, inttostr(fLinesPerPage)); //2022.03.27 LBZ
  end;
end;
...
begin
  ...
  XML.SetValue(Path+'LinesPerPage/Value', fLinesPerPage); //2022.03.27 LBZ
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      LinesPerPages := TfrBandView(bt).fLinesPerPage;//2022.03.27 LBZ
      Flag := True;
    end;
    //2022.03.27 LBZ  Begin
    if (bt.Typ = gtBand) and
       (TfrBandView(bt).BandType in [btMasterData]) then
    with Bands[TfrBandView(bt).BandType] do
    begin
      Memo.Assign(bt.Memo);
      Script.Assign(bt.Script);
...
begin
  ...
  BaseName:='Page';
  fWaterMarkText:='';//2022.06.14 LBZ
  List := TFpList.Create;
  Objects := TFpList.Create;
  RTObjects := TFpList.Create;
  //2022.04.18 LBZ 增加默认页边距 20mm
  fMargins.Left :=72;
  fMargins.Top:=72;
  fMargins.Right:=72;
  fMargins.Bottom:=72;
...
var
  ...
  var
    WasPrinted: Boolean;
    b, b1, b2: TfrBand;
    i: Integer;   //2022.03.15 LBZ

    //2022.03.15 LBZ
    procedure clearBand(bb1:TfrBand;disptext:Boolean);
    var i:Integer;
    begin
      for i := 0 to bb1.Objects.Count-1 do
      begin
        if TfrView(bb1.Objects[i]) is TfrMemoView then
        begin
          TfrMemoView(TfrView(bb1.Objects[i])).Memo.Clear;
          if disptext then
          begin
            if TfrMemoView(TfrView(bb1.Objects[i])).FBlankDispText<>'' then
            begin
              TfrMemoView(TfrView(bb1.Objects[i])).Alignment:=taCenter;//水平居中
              TfrMemoView(TfrView(bb1.Objects[i])).Layout:=tlCenter;//垂直居中
              TfrMemoView(TfrView(bb1.Objects[i])).Memo.Text:=
              TfrMemoView(TfrView(bb1.Objects[i])).FBlankDispText;
            end;
          end;
        end;
      end;
  ...
  begin
    ...
    begin
      ...
      try
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              Inc(b.Positions[psGlobal]);
              Inc(b.Positions[psLocal]);
              // 2022.03.27 LBZ 设定每行行数 BEGIN
              Inc(b.FNowLine);
              // 2022.03.27 LBZ 设定每行行数
              if (b.LinesPerPages <> 0) and (b.FNowLine > b.LinesPerPages) and (not b.DataSet.Eof) then
              begin
                b.FNowLine := 0; // 当行数>设定行数时，换页
              end;
              //2022.03.27 LBZ 设定每行行数 END
              // 一个记录一页（强制换页）
              if (b.LinesPerPages = 0) and not b.DataSet.Eof and b.NewPageAfter then
              begin
                b.FNowLine := 0;
              end;
          ...
          //2022.03.27 LBZ  自动增加空行
          if b.Typ=btMasterData then
          begin
             if  (b.LinesPerPages > 1) then
             begin
               B.Script.Clear;
               clearBand(b,True);//添加空行前先清空原band的内容，并显示TfrMemoView.BlankDispText预留的文字
               for i := 1 to b.LinesPerPages-b.FNowLine do
               begin
                 ShowBand(b);
                 if i=1 THEN clearBand(b,False); //添加空行前先清空原band的内容，除第1行空行外，其他空行不显示TfrMemoView.BlankDispText预留的文字
               end;
             end;
          end;
          //2022.03.27 LBZ  自动增加空行
        ...
        // 当数据集为空，且为自动增加空行时，自动添加空行 LBZ 2003-1-29
        //2022.03.27 LBZ  自动增加空行
        if  (b.LinesPerPages > 1) then
        begin
          B.Script.Clear;
...
{  b := XML.GetValue(Path+'PageType/Value'{%H-}, ord(PageType));
  ...
  fWaterMarkText:=XML.GetValue(Path+'WaterMarkText/Value', '');//2022.06.14 LBZ
...
//  XML.SetValue(Path+'PageType/Value'{%H-}, ord(PageType));
  ...
  XML.SetValue(Path+'WaterMarkText/Value', fWaterMarkText);//2022.06.14 LBZ
end;
...
begin
  ...
  WaterMark.y := 50;
  WaterMark.Angle :=0;
  WaterMark.Font := TFont.Create;
  WaterMark.Font.Color := clSilver;
  WaterMark.Font.Size := 18;
  //LBZ END
 {$ENDIF}
end;
...
begin
  ...
  //2022.5.20 LBZ  修复linux下变量大小写敏感的问题，统一将s转为大写(ss.ToUpper)
  DoGetValue(s.ToUpper,aValue);
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                //raise(EParserError.Create(SE1 + 'Undefined symbol: ' + SubValue));//2022.05.19 LBZ取消
              end;
            end;
          end;
        end;
...
begin
  ...
  begin
    ...
    frPrintForm.RB1.Enabled:=true;
    frPrintForm.RB2.Enabled:=false;
    frPrintForm.RB3.Enabled:=True;
    frPrintForm.E1.Value:=DefaultCopies;
    frPrintForm.cbCollate.Checked:=DefaultCollate;
    frPrintForm.E2.TextHint:='本栏空白时，打印全部页。';
    if frPrintForm.ShowModal = mrOk then
    begin
      if frPrintForm.RB1.Checked then
        PageNumbers := ''
      else
        PageNumbers := frPrintForm.E2.Text;
      PrepareReport;
      PrintPreparedReport(PageNumbers,frPrintForm.E1.Value);
    end;
    frPrintForm.Free;
  end
  else
    MessageDlg('出错提醒','未安装打印机,请安装打印机后再打印！',mtWarning,[mbOK],0);
    //ShowMessage('未安装打印机,请安装打印机后再打印！');
end;
//LBZ 2022.06.12
...
var
  ...
  var
    rotatedFont:TFont; //2022.06.14 LBZ
    _WaterMarkText:string;//2022.06.14 LBZ
  begin
    {$ifdef DebugLR}
    ...
    begin
      ...
      begin
        ...
        begin
          if WaterMark.X <= 0 then WaterMark.X := 50;
          if WaterMark.Y <= 0 then WaterMark.Y := 50;
          Canvas.Font:=WaterMark.Font;
          rotatedFont := TFont.Create;
          rotatedFont.assign(Canvas.Font);
          rotatedFont.Orientation := WaterMark.Angle*10; //旋转指定角度
          Canvas.Font := rotatedFont;
          Canvas.Brush.Style := bsClear;
          Canvas.TextOut(round(WaterMark.X / (25.4 / 96)), round(WaterMark.Y / (25.4 / 96)), _WaterMarkText);
          rotatedFont.Free;
        end;
        //LBZ END 2022.6.14  指定位置打印特定字符，一般用于版本等信息
        if pgMargins then
