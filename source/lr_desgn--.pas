...
type
  ...
  TfrDesignerForm = class(TfrReportDesigner)
    ...
    frTBSeparator17: TPanel;
    frTBSeparator18: TPanel;
...
var
  ...
  procedure mvL(x, y, dx, kx, K: integer); //左
  var i: integer;
    t: tfrview;
  begin
    for i := 0 to objects.Count - 1 do
    begin
      ...
      if K = 1 then
      begin
        if (y = t.y) and (t.x + t.dx = x) then t.dx := t.dx + kx;
        if (y = t.y) and (t.x = x) and (t.dx = dx) then
        begin
          t.x := t.x + kx;
          t.dx := t.dx - kx;
        end;
      ...
      if K = 0 then
      begin
        if (t.x + t.dx = x) then t.dx := t.dx + kx;
        if (t.x = x) then
        begin
          t.x := t.x + kx;
          t.dx := t.dx - kx;
  ...
  procedure mvR(x, y, dx, kx, K: integer); //右
  var i: integer;
    t: tfrview;
  begin
    for i := 0 to objects.Count - 1 do
    begin
      ...
      if k = 1 then
      begin
        if (y = t.y) and (t.x = x + dx) then
        begin
          t.dx := t.dx - kx;
          t.x := t.x + kx;
        end;
        if (y = t.y) and (t.x = x) and (t.dx = dx) then t.dx := t.dx + kx;
      end;
      if k = 0 then
      begin
        if (t.x = x + dx) then
        ...
        if t.x + t.dx = dx + x then t.dx := t.dx + kx;
      end;
    end;
    FDesigner.RedrawPage;
  end;

  procedure mvT(x, y, dy, ky, K: integer); //上
  var i: integer;
    t: tfrview;
  begin
    for i := 0 to objects.Count - 1 do
...
//        if (t.y + t.dy = y) and ((t.x>=x) and (t.x+t.dx<x)) then t.dy := t.dy + ky;
        if (t.y + t.dy = y) and (t.x = x) then t.dy := t.dy + ky;
      end;
      if k = 0 then
      begin
        if (t.y = y) then
        ...
        if (t.y + t.dy = y) then t.dy := t.dy + ky;
      end;
    end;
    FDesigner.RedrawPage;
{
    FDesigner.PageView.GetMultipleSelected;
    FDesigner.PageView.Draw(TopSelected, ClipRgn);
}
  end;

  procedure mvB(x, y, dy, ky, K: integer); //下
  var i: integer;
    t: tfrview;
  begin
    for i := 0 to objects.Count - 1 do
    begin
      ...
      if k = 1 then
      begin
        if (y + dy = t.y) and (t.x = x) then
        begin
          t.y := t.y + ky;
          t.dy := t.dy - ky;
        end;
        if (t.y = y) and (t.x = x) then t.dy := t.dy + ky;
      end;
      if k = 0 then
      begin
        if (y + dy = t.y) then
        ...
        if (t.y + t.dy = y + dy) then t.dy := t.dy + ky;
      end;
    end;
    FDesigner.RedrawPage;
{
...
{$ENDIF}
  ...
  begin
    ...
    begin
     ...
     if Cursor = crSizeWE then //水平调整单元格尺寸 2022-03-28 LBZ
    begin
      if (CT <> ct6) and ((CT = ct5) or Cont(t.x, t.y + t.dy div 2, LastX, LastY)) then
      begin
        if (GetKeyState(VK_SHIFT) < 0) then
          mvL(t.x, t.y, t.dx, kx, 0) //左
        else
          mvL(t.x, t.y, t.dx, kx, 1); //左
      ...
      begin
        if (GetKeyState(VK_SHIFT) < 0) then
          mvR(t.x, t.y, t.dx, kx, 0) //右
        else
          mvR(t.x, t.y, t.dx, kx, 1);
    ...
    if Cursor = crSizeNS then//垂直调整单元格尺寸 2022-03-28 LBZ
    begin
      if (CT <> ct8) and ((CT = ct7) or Cont(t.x + t.dx div 2, t.y, LastX, LastY)) then
      begin
        if (GetKeyState(VK_SHIFT) < 0) then
          mvT(t.x, t.y, t.dy, ky, 0) //上
        else
          mvT(t.x, t.y, t.dy, ky, 1); //上
        if CheckNegative(t) then
          CT := ct8 else
          CT := ct7;
      end
      ...
      begin
        if (GetKeyState(VK_SHIFT) < 0) then
          mvB(t.x, t.y, t.dy, ky, 0) //下
        else
          mvB(t.x, t.y, t.dy, ky, 1); //下
        if CheckNegative(t) then
          CT := ct7 else
          CT := ct8;
      end;
...
//  FirstInstance := FirstInst;
  ...
  //EditorForm := TfrEditorForm.Create(nil);//2022.12.10 LBZ修正编辑文本后退出lazarus出错
  ...
  IEPopupMenu.Parent:=PageView;
{
  ...
  CurReport.ReportCreateDate:=Now;//2022.06.17 LBZ 设置报表创建日期
end;
...
begin
  ...
  Panel6.Height := 26;
  Panel6.Width := 26;
  Panel6.Height := Panel2.Height; //2022.04.17 LBZ  调整xls导入和插入数据库字段图标大小
  Panel6.Width := Panel2.Height;  //2022.04.17 LBZ  调整xls导入和插入数据库字段图标大小
  ...
  begin
    ...
    Align:=alLeft;
//    Align:=alTop;
    SetBounds(W, 1, Panel6.Height, Panel6.Height); //2022.04.17 LBZ  //SetBounds(W, 1, 22, 22);
    Visible:=True;
    ShowHint:=True;
    Tag := 36;
  end;
...
procedure TfrDesignerForm.SpeedButton1Click(Sender: TObject);
var v: tfrview;
  //page1: tfrpage;
  k, ls, k3, k1, k2, kkk, i, j: integer;
  hs, oldx, oldy, x, y, x1, y1, dx, dy, dx1, dy1: integer;
  gr: array of array of integer;
  gr2, gr1: array of integer;
  hs1, ls1: array[0..1000] of integer;
  err:Integer;

  procedure _Sort(var a: array of integer);
  var i, j, k, n: integer;
    d: integer;
  begin
    n := high(a) + 1;
  ...
  function jshs(s: integer): integer;
  var i, j: integer;
  begin
    j := 0;
    jshs := 0;
  ...
  function jsls(s: integer): integer;
  var i, j: integer;
  begin
    j := 0;
    jsls := 0;
  ...
  function hwz(s: integer): integer; //行数
  var i: integer;
  begin
    hwz := 0;
    for i := 0 to 1000 do
  ...
  function lwz(s: integer): integer; //列数
  var i: integer;
  begin
    lwz := 0;
    for i := 0 to 1000 do
...
//  exitedit;
  ...
  begin
    ...
    begin
      ...
      dy1 := jshs(v.y); //行数
      dx1 := jsls(v.x); //列数
      if (x <> 0) and (x > v.x) then x := v.x;
      if (x1 < v.x) then x1 := v.x;
      if (y <> 0) and (y > v.y) then y := v.y;
      if (y1 < v.y) then y1 := v.y;
    end;
  end;
  if kkk = 1 then
  begin
    setlength(gr, dy1); //行
    for i := low(gr) to high(gr) do
    begin
      setlength(gr[i], dx1);
      for j := low(gr[i]) to high(gr[i]) do
        gr[i, j] := -1;
    end;
    hs := dy1; //行数
    ls := dx1; //列数
    try
      ...
      begin
        v := tfrView(Objects[i]);
        if v.Selected then gr[hwz(v.y), lwz(v.x)] := i; //行、列
      end;
      for j := 0 to high(gr[0]) do //列
      begin
        for i := low(gr) to high(gr) do //行
        begin
           if gr[i,j]<0 Then
           Begin
             err:=1;
      ...
      begin
        ...
        setlength(gr1, high(gr) + 1);
        setlength(gr2, high(gr) + 1);
        ...
        begin
          ...
          for i := low(gr2) to high(gr2) do //行
          begin
            v := tfrView(objects[gr2[i]]);
            if i = 0 then y := oldy
            else y := y + k1 div k2;
            k3 := k3 + k1 div k2;
            v.y := y;
            if (i = high(gr2)) and (k3 <> k1) then v.dy := (k1 div k2) + (k1 - k3)
            else v.dy := k1 div k2;
          end;
        end;
        gr1 := nil;
...
procedure TfrDesignerForm.SpeedButton2Click(Sender: TObject);
var v: tfrview;
  //page: tfrpage;
  k, ls, k3, k1, k2, kkk, i, j: integer;
  hs, oldx, oldy, x, y, x1, y1, dx, dy, dx1, dy1: integer;
...
begin
  ...
  begin
    ...
    try
      ...
      for i := 0 to hs - 1 do //行
      begin
        for j := 0 to ls - 1 do //列
        begin
          if gr[i,j]<0 THEN
          begin
            err:=1;
      ...
      begin
        ...
        begin
          ...
          for j := low(gr2) to high(gr2) do //列
          begin
            v := tfrview(objects[gr2[j]]);
            if j = 0 then x := oldx
            else x := x + k1 div k2;
            k3 := k3 + k1 div k2;
            v.x := x;
            if (j = high(gr[i])) and (k3 <> k1) then v.dx := (k1 div k2) + (k1 - k3)
            else v.dx := k1 div k2;
          end;
        end;
        gr1 := nil;
...
procedure TfrDesignerForm.SpeedButton3Click(Sender: TObject);
var v: tfrview;
  ii, i, j, k, h, v1, e: integer;
  x, y, dx, dy, x1, y1, dx1, dy1, dy2: integer;
  M1: TStringList;
  fs: tfont;
  ali: TAlignment;
  fb: TfrFrameBorders;
...
begin
  selnum := 0;
  page := CurReport.pages[CurPage];
  AUTOTABLEFORM := TAUTOTABLEFORM.Create(nil);
  AUTOTABLEFORM.showmodal;
  val(AUTOTABLEFORM.Edit1.text, h, e); //行
  val(AUTOTABLEFORM.Edit2.text, v1, e); //列
  freeandnil(AUTOTABLEFORM);
  ii := 0;
  for k := 0 to objects.Count - 1 do
  begin
    x := 0;
    y := 0;
    dx := 0;
    ...
    begin
      ...
      dy := v.dy;
      m1.Assign(v.memo);
      ali := TfrmemoView(v).Alignment;
      fb:=TfrmemoView(v).Frames;
      fs := TfrMemoView(v).font;
      ly:=TfrMemoView(v).Layout;
      x1 := x;
      y1 := y;
      dx1 := dx div v1;
      dy1 := dy div h;
      dy2 := abs(dy1 + (dy - dy1 * h)); //行结束
      for i := 1 to h do //行
      begin
        y := y1 + (i - 1) * dy1;
        if i = h then dy1 := dy2; //行结束
        for j := 1 to v1 do //列
        begin
          v := frCreateObject(gtMemo, '',nil);
          v.CreateUniqueName; //自动生成单元名称
          x := x1 + (j - 1) * dx1;
          if j = v1 then dx1 := abs(dx1 + (dx - dx1 * j)) //列结束
          else dx1 := dx div v1;
          v.SetBounds(x, y, dx1, dy1);
          TfrmemoView(v).Alignment := ali;
          TfrmemoView(v).Layout:=ly;
          TfrmemoView(v).Frames:=fb;
          TfrMemoView(v).font := fs;
           if (i = 1) and (j = 1) then v.Memo.Assign(m1);
          inc(selnum);
          Objects.Add(v);
        end;
      end;
...
procedure TfrDesignerForm.SpeedButton4Click(Sender: TObject);
label go;
var v: tfrview;
  kkk, i, j, m: integer;
  oldx, oldy, x, y, x1, y1, dx, dy, dx1, dy1: integer;
  hs, ls: integer;
  ali:TAlignment;
  fb: TfrFrameBorders;
  ly: TTextLayout;
...
begin
  ...
  begin
    ...
    try
      ...
      for i := low(gr) to high(gr) do //行
      begin
        for j := low(gr[i]) to high(gr[i]) do //列
        begin
          v := tfrView(objects[gr[i, j]]);
          if i = 0 then dx := dx + v.dx;
          if j = 0 then dy := dy + v.dy;
          for m := 0 to v.Memo.Count - 1 do
          begin
            m1.Add(v.Memo.Strings[m]);
          end;
        end;
      end;
      v := frcreateobject(gtMemo, '',nil);
      v.CreateUniqueName; //自动生成单元名称
      v.SetBounds(oldx, oldy, dx, dy);
      TfrmemoView(v).Alignment := ali;
      TfrmemoView(v).Frames := fb;
      TfrmemoView(v).Layout := ly;
      TfrMemoView(v).font := fs;
...
begin
  ...
  begin
    ...
    Alignment:=taLeftJustify;//2022.06.17 LBZ 将标题名左对齐
    ...
    Top := 0;
    Height := 32; //18 LBZ 2022.06.18 修改标题高度
  ...
  begin
    ...
    DefaultItemHeight := fcboxObjList.Height+10; //2022.06.17 LBZ fcboxObjList.Height-3; 调整列表的行高度
  end;
  {$ENDIF}
end;
